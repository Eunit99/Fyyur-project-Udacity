...
def venues():
    ...
    for venue in all_venues:
        # save the cities and states as tuples in the declared set
    ...
    for venue in all_venues:
        for city_object in cities:
            if venue.city == city_object.get('city') and venue.state == city_object.get('state'):
                ...
                # find all shows happening in the future with a start time greater down now
                upcoming_shows = len(Show.query.join(Venue).filter(
                    Show.start_time > datetime.datetime.utcnow()).all())
...
def search_venues():
    ...
    search_result = Venue.query.filter(Venue.name.ilike(
        '%' + search_query + '%')).all()  # get venue by query results
    search_array = []

    for data in search_result:
        num_upcoming_shows = Show.query.filter(
            Show.venue_id == data.id).count()  # get number of upcoming shows
        search_array.append({
            "id": data.id,
            "name": data.name,
...
def show_venue(venue_id):
    ...
    past_shows_all = Show.query.join(Venue, Show.venue_id == venue_id).filter(
        Show.start_time <= datetime.datetime.utcnow()).all()
    upcoming_shows_all = Show.query.join(Venue, Show.venue_id == venue_id).filter(
        Show.start_time > datetime.datetime.utcnow()).all()
...
def create_venue_submission():
    ...
    if venue_form.validate():
        ...
        try:
            new_venue = Venue(name=name, description=seeking_description, city=city, state=state, address=address, phone=phone,
                              genres=genres, facebook_link=facebook_link, website_link=website_link, image_link=image_link,  is_talent=seeking_talent)
        ...
        # TODO: on unsuccessful db insert, flash an error instead.
            flash('An error occurred. Venue ' + new_venue.name +
...
def search_artists():
    ...
    search_result = Artist.query.filter(Artist.name.ilike(
        '%' + search_query + '%')).all()  # get artist by query results
    search_array = []
    for data in search_result:
        search_array.append({
            "id": data.id,
...
def show_artist(artist_id):
    ...
    past_shows_all = Show.query.join(Artist, Show.artist_id == artist.id).filter(
        Show.start_time <= datetime.datetime.utcnow()).all()
    upcoming_shows_all = Show.query.join(Artist, Show.artist_id == artist.id).filter(
        Show.start_time > datetime.datetime.utcnow()).all()
...
def create_artist_submission():
    ...
    if artist_form.validate():
        # TODO: modify data to be the data object returned from db insertion
        try:
            new_artist = Artist(name=name, description=seeking_description, city=city, state=state, phone=phone, genres=genres,
                                facebook_link=facebook_link, website_link=website_link, image_link=image_link,  is_venue=seeking_venue)
